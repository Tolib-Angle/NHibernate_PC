...
namespace PublishingCenter_v2.PublishingCenter.Tables
{
    public class TabBooks
    {
        ...
        private static void ShowPrev()
        {
            ...
            templateInterface.SetNewId(books[0].id, books[books.Count - 1].id);
            //books = Enumerable.Reverse(books).ToList();
        }
        ...
        private static void Delete()
        {
            ...
            booksItem = TemplateInterface<Books>.GetDataByNumber(delete_number - 1);
            string all_writers = AllWriters(booksItem.writers);
            Console.WriteLine("+--------+------------+----------------------------------------+----------------------------------------+------------+------------+------------+------------+------------+--------------------------------------------------+");
            Console.WriteLine("|{0, -8}|{1, -12}|{2, -40}|{3, -40}|{4, -12}|{5, -12}|{6, -12}|{7, -12}|{8, -12}|{9, -50}|", "N", "Cepher book", "Pub. name", "Title", "Circulation", "Date R.", "Cost price", "Sale price", "Fee", "Authors");
            Console.WriteLine("+--------+------------+----------------------------------------+----------------------------------------+------------+------------+------------+------------+------------+--------------------------------------------------+");
        ...
        private static void Update()
        {
            ...
            Books booksItem = new Books();
            booksItem = TemplateInterface<Books>.GetDataByNumber(number - 1);
            string all_writers = AllWriters(booksItem.writers);
            Console.WriteLine("+--------+------------+----------------------------------------+----------------------------------------+------------+------------+------------+------------+------------+--------------------------------------------------+");
            Console.WriteLine("|{0, -8}|{1, -12}|{2, -40}|{3, -40}|{4, -12}|{5, -12}|{6, -12}|{7, -12}|{8, -12}|{9, -50}|", "N", "Cepher book", "Pub. name", "Title", "Circulation", "Date R.", "Cost price", "Sale price", "Fee", "Authors");
            Console.WriteLine("+--------+------------+----------------------------------------+----------------------------------------+------------+------------+------------+------------+------------+--------------------------------------------------+");
            ...
            while (!end_update)
            {
                ...
                else if (parametr == "Authors")
                {
                    ...
                    while (!ext)
                    {
                        ...
                        if (str == "Add")
                        {
                            booksItem.writers.Add(TemplateInterface<Writers>.GetDataByNumber(TemplateHelpFunctions.EnterNumber(1, Repository.Instance.GetCount<Writers>())));
                        }
                        else if (str == "Delete")
                        {
                            booksItem.writers.Remove(TemplateInterface<Writers>.GetDataByNumber(TemplateHelpFunctions.EnterNumber(1, Repository.Instance.GetCount<Writers>())));
                        }
                        else if (str == "Exit")
                            ext = true;
                        else
        ...
        private static void Add()
        {
            ...
            while (!exit)
            {
                ...
                if (str == "Add")
                {
                    ...
                    book.writers.Add(TemplateInterface<Writers>.GetDataByNumber(writer));
                }
        ...
        private static void Search()
        {
            ...
            while (!exit)
            {
                ...
                if (action == "Search")
                {
                    ...
                    string Hql = $"from Books as e where " +
                    ...
                    List<Books> books_tmp = TemplateInterface<Books>.SearchDataByText(Hql, str);
