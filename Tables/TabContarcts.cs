...
namespace PublishingCenter_v2.PublishingCenter.Tables
{
    public class TabContarcts
    {
        ...
        private static void Update()
        {
            ...
            contract = TemplateInterface<Contracts>.GetDataByNumber(number - 1);
            Console.WriteLine("+--------+------------+-----------------------------------+------------+------------+------------+------------+");
            Console.WriteLine("|{0, -8}|{1, -12}|{2, -35}|{3, -12}|{4, -12}|{5, -12}|{6, -12}|", "N", "Contract N", "Writer", "Date C.", "Term", "Validy", "Date T.");
            Console.WriteLine("+--------+------------+-----------------------------------+------------+------------+------------+------------+");
            ...
            while (!end_update)
            {
                ...
                {
                    contract.writer = TemplateInterface<Writers>.GetDataByNumber(TemplateHelpFunctions.EnterNumber(1, Repository.Instance.GetCount<Writers>()));
                    //int new_id_writer = TemplateHelpFunctions.EnterNumber(1, Repository.Instance.GetCount<Writers>());
                    //contract.writer = TemplateInterface<Writers>.GetDataByNumber(new_id_writer + 1);
                }
        ...
        private static void Delete()
        {
            ...
            Contracts contract = new Contracts();
            contract = TemplateInterface<Contracts>.GetDataByNumber(delete_number - 1);
            Console.WriteLine("+--------+------------+-----------------------------------+------------+------------+------------+------------+");
            Console.WriteLine("|{0, -8}|{1, -12}|{2, -35}|{3, -12}|{4, -12}|{5, -12}|{6, -12}|", "N", "Contract N", "Writer", "Date C.", "Term", "Validy", "Date T.");
            Console.WriteLine("+--------+------------+-----------------------------------+------------+------------+------------+------------+");
        ...
        private static void Add()
        {
            ...
            contract.writer = TemplateInterface<Writers>.GetDataByNumber(id_writer + 1);
        ...
        private static void Search()
        {
            ...
            while (!exit)
            {
                ...
                if (action == "Search")
                {
                    ...
                    string Hql = $"from Contracts as e where " +
                    ...
                    List<Contracts> contracts_tmp = TemplateInterface<Contracts>.SearchDataByText(Hql, str);
